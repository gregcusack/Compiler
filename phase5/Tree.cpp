...
void Function::generate() {
    ...
    //all symbols in a list
    Symbols slist = decls->symbols();
    Parameters *p = _id->type().parameters();
    unsigned i;
    int offset = 0;

    //set offset for each param in function
    for(i=0; i < p->size(); i++) {
	slist[i]->setOffset(8+i*4);  //_offset = 8+i*4;
    }

    //set offset for each local var in a function
    for(;i<slist.size(); i++) {
	offset -= slist[i]->type().size();
	slist[i]->setOffset(offset);
    }

    //cout << "In function generate" << endl;
    //Prologue
